...
async function deleteMessage(messageId) {
  try {
    // 保存當前捲動位置信息
    const isAtBottom = shouldScrollToBottom();
    ...
    // 刪除訊息
    const updatedHistory = await StorageUtils.deleteMessageFromHistory(messageId);
    currentChatHistory = updatedHistory; // 更新全局變數
    ...
    if (!Array.isArray(updatedHistory)) {
      console.error('刪除後返回的歷史記錄不是有效數組');
      UiUtils.showError('刪除訊息後獲取歷史記錄失敗');
      return;
    }
    ...
    setTimeout(() => {
      try {
        if (isAtBottom) {
          ...
          // 否則，恢復相對捲動位置
          const newScrollPosition = scrollPercentage * (chatContainer.scrollHeight - chatContainer.clientHeight);
          chatContainer.scrollTop = newScrollPosition;
        }
      } catch (scrollError) {
        console.error('調整捲動位置時出錯:', scrollError);
      }
    }, 50);
    ...
    // 如果是 DOMException，顯示更具體的錯誤信息
    if (error instanceof DOMException) {
      console.error('刪除訊息時發生 DOM 異常:', error.name, error.message);
      UiUtils.showError(`刪除訊息時發生 DOM 異常: ${error.name}`);
    } else {
      UiUtils.showError(`刪除訊息時出錯: ${error.message}`);
    }
  }
}
